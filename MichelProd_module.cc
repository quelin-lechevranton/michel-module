...
void ana::MichelProd::produce(art::Event& e)
{
  ...
  for (PtrTrk const& pt_ev: vpt_ev) {
    ...
    if (geoDet == kPDVD) {
      ...
      size_t front_hit_track_idx = vhm_trk.at(map_hitkey2idx[sh_mu.start().key()])->Index();
      float front_hit_y = pt_ev->HasValidPoint(front_hit_track_idx)
        ? pt_ev->LocationAtPoint(front_hit_track_idx).Y()
        : util::kBogusF;
      size_t back_hit_track_idx = vhm_trk.at(map_hitkey2idx[sh_mu.end().key()])->Index();
    ...
    size_t end_track_idx = vhm_trk.at(map_hitkey2idx[sh_mu.end().key()])->Index();
